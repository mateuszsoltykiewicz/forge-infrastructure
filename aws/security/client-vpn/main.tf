...
resource "aws_iam_role" "vpn_cloudwatch_logs" {
  ...
  name_prefix = substr("${local.vpn_name}-cw-logs-", 0, 38)
...
resource "aws_iam_role_policy" "vpn_cloudwatch_logs" {
  ...
  role        = aws_iam_role.vpn_cloudwatch_logs.id
    ...
    Statement = [
      {
        ...
        Resource = "${aws_cloudwatch_log_group.vpn_connection_logs.arn}:*"
      }
    ]
  })
}
...
resource "aws_ec2_client_vpn_endpoint" "this" {
  ...
  # Security Groups
  security_group_ids = [module.vpn_security_group.security_group_id]
  ...
  connection_log_options {
    ...
    cloudwatch_log_group  = var.enable_connection_logs ? aws_cloudwatch_log_group.vpn_connection_logs.name : null
    cloudwatch_log_stream = var.enable_connection_logs ? aws_cloudwatch_log_stream.vpn_connection_stream.name : null
  }
  ...
  depends_on = [
    ...
    aws_iam_role.vpn_cloudwatch_logs,
    aws_iam_role_policy.vpn_cloudwatch_logs,
    aws_cloudwatch_log_group.vpn_connection_logs,
    aws_cloudwatch_log_stream.vpn_connection_stream,
...
resource "aws_ec2_client_vpn_network_association" "this" {
  count = length(module.client_vpn_subnets.subnet_ids)

  client_vpn_endpoint_id = aws_ec2_client_vpn_endpoint.this.id
  subnet_id              = module.client_vpn_subnets.subnet_ids[count.index]
...
resource "aws_ec2_client_vpn_route" "vpc_route" {
  ...
  description            = "Route to VPC CIDR block via subnet ${module.client_vpn_subnets.subnet_ids[count.index]}"
